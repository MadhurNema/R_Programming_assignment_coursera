...
rankhospital <- function(state,outcome,num){
  ...
  # To change the class of the columns to numeric & suppress the warnings
  ...
  if(outcome == "heart attack"){
    outcome_dt <-outcome_state[order(outcome_state$Hospital.30.Day.Death..Mortality..Rates.from.Heart.Attack,outcome_state$Hospital.Name),]
    if(num == "best"){
      outcome_name <- outcome_dt[,"Hospital.Name"][1]
    }else if(num == "worst"){
      outcome_name <- outcome_dt[,"Hospital.Name"][nrow(outcome_state)]
    }else{
      outcome_name <- outcome_dt[,"Hospital.Name"][num]
    }
  }else if(outcome == "heart failure"){
    outcome_dt <-outcome_state[order(outcome_state$Hospital.30.Day.Death..Mortality..Rates.from.Heart.Failure,outcome_state$Hospital.Name),]
    if(num == "best"){
      outcome_name <- outcome_dt[,"Hospital.Name"][1]
    }else if(num == "worst"){
      outcome_name <- outcome_dt[,"Hospital.Name"][nrow(outcome_state)]
    ...
    outcome_dt <-outcome_state[order(outcome_state$Hospital.30.Day.Death..Mortality..Rates.from.Pneumonia,outcome_state$Hospital.Name),]
    if(num == "best"){
      outcome_name <- outcome_dt[,"Hospital.Name"][1]
    }else if(num == "worst"){
      outcome_name <- outcome_dt[,"Hospital.Name"][nrow(outcome_state)]
